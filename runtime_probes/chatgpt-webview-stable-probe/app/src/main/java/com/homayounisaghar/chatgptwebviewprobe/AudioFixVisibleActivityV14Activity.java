...
/**
 * Stable v0.14: one-variable audio fix experiment plus a read-only dashboard
 * activity probe. MODIFY_AUDIO_SETTINGS is the only new audio intervention.
 * Visible progress/status text may be displayed transiently but is never written
 * to the report. Hidden model chain-of-thought is neither requested nor accessed.
 */
public class AudioFixVisibleActivityV14Activity extends AudioEarlyTraceV13Activity {
    ...
    private boolean docStartSupported;
    private boolean docStartSeen;
    private boolean assistantActive;
    private boolean stopPresent;
    private int bridgeMessages;
    private int assistantChanges;
    private int statusChanges;
    private int statusNodes;
    private int lastAssistantLen;
    private long lastAssistantChangeAt;
    private String lastAssistantSig = "-";
    private String lastStatusHash = "-";
    private int lastStatusLen;
    private String lastStatusKind = "-";
    ...
    @Override public void onCreate(Bundle savedInstanceState) {
        ...
        retitle(getWindow().getDecorView());
        addStatusPanel();
    ...
    private void retitle(View v) {
        ...
        if (v instanceof ViewGroup) {
            ...
            for (int i = 0; i < g.getChildCount(); i++) retitle(g.getChildAt(i));
        }
    }
    ...
    private void addStatusPanel() {
        ...
        status14.setPadding(dp(7), dp(2), dp(7), dp(2));
    ...
    private void installActivityProbe() {
        ...
        h14.postDelayed(() -> {
            if (web14 != null) web14.evaluateJavascript(ACTIVITY_JS,
                    value -> ev14("ACTIVITY_LATE_FALLBACK_DISPATCHED"));
        }, 300L);
    }
        ...
        try {
            ...
            switch (type) {
                ...
                case "ASSISTANT_CHANGE":
                    ...
                    lastAssistantSig = tok(d.optString("sig", "-"));
                    lastAssistantChangeAt = System.currentTimeMillis();
                    break;
                case "VISIBLE_STATUS":
                    statusChanges++;
                    statusNodes = d.optInt("nodes", 0);
                    lastStatusKind = tok(d.optString("kind", "-"));
                    String raw = d.optString("text", "");
                    if (raw.length() > 220) raw = raw.substring(0, 220);
                    transientStatus = raw;
                    lastStatusLen = raw.length();
                    lastStatusHash = raw.isEmpty() ? "-" : sha256(raw);
                    break;
                case "ACTIVITY_STATE":
                    assistantActive = d.optBoolean("active", false);
                    stopPresent = d.optBoolean("stop", false);
                    statusNodes = d.optInt("statusNodes", statusNodes);
                    break;
                default:
                    break;
            }
            if (events14.length() < 22000) {
                events14.append(stamp()).append(" | ").append(type)
                        ...
                        .append(" kind=").append(tok(d.optString("kind", "-"))).append('\n');
            }
            render14();
        } catch (Exception e) {
    ...
    private void render14() {
        ...
        long age = lastAssistantChangeAt == 0L ? -1L
                : Math.max(0L, System.currentTimeMillis() - lastAssistantChangeAt);
        String visible = transientStatus.isEmpty() ? "(none)" : transientStatus;
        StringBuilder s = new StringBuilder();
        s.append("v0.14 AUDIO FIX + VISIBLE ACTIVITY\n");
        s.append("MODIFY_AUDIO_SETTINGS=")
                .append(checkSelfPermission(Manifest.permission.MODIFY_AUDIO_SETTINGS)
                        == PackageManager.PERMISSION_GRANTED ? "GRANTED" : "DENIED")
                .append(" DocStart=").append(docStartSeen)
                .append(" Active=").append(assistantActive)
                .append(" Stop=").append(stopPresent).append('\n');
        s.append("AssistantChanges=").append(assistantChanges)
                .append(" lastAgeMs=").append(age)
                .append(" statusNodes=").append(statusNodes)
                .append(" statusChanges=").append(statusChanges).append('\n');
    ...
    private String reportText14() {
        ...
        r.append("MODIFY_AUDIO_SETTINGS_PERMISSION=")
                .append(checkSelfPermission(Manifest.permission.MODIFY_AUDIO_SETTINGS)
                        == PackageManager.PERMISSION_GRANTED ? "GRANTED" : "DENIED").append('\n');
        r.append("AUDIO_FIX_INTERVENTION=MANIFEST_PERMISSION_ONLY\n");
        r.append("ACTIVITY_WEB_MESSAGE_LISTENER_INSTALLED=").append(bridgeInstalled).append('\n');
        r.append("ACTIVITY_DOCUMENT_START_SUPPORTED=").append(docStartSupported).append('\n');
        r.append("ACTIVITY_DOCUMENT_START_SEEN=").append(docStartSeen).append('\n');
        r.append("ACTIVITY_BRIDGE_MESSAGES=").append(bridgeMessages).append('\n');
        r.append("ASSISTANT_ACTIVITY_ACTIVE=").append(assistantActive).append('\n');
        r.append("STOP_CONTROL_PRESENT=").append(stopPresent).append('\n');
        r.append("ASSISTANT_CHANGE_EVENTS=").append(assistantChanges).append('\n');
        ...
        r.append("VISIBLE_STATUS_NODE_COUNT=").append(statusNodes).append('\n');
        r.append("VISIBLE_STATUS_CHANGES=").append(statusChanges).append('\n');
        r.append("LAST_VISIBLE_STATUS_KIND=").append(lastStatusKind).append('\n');
        r.append("LAST_VISIBLE_STATUS_LENGTH=").append(lastStatusLen).append('\n');
        r.append("LAST_VISIBLE_STATUS_SHA256=").append(lastStatusHash).append('\n');
    ...
    private void ev14(String s) {
        if (events14.length() < 22000) events14.append(stamp()).append(" | ").append(tok(s)).append('\n');
    }
    ...
    private static final String ACTIVITY_JS =
            ...
            + "const stopNow=()=>[...document.querySelectorAll('button,[role=button]')].some(e=>{if(!V(e))return false;const z=(N(e.getAttribute('aria-label'))+' '+N(e.getAttribute('data-testid'))+' '+N(e.textContent)).toLowerCase();return z.includes('stop generating')||z==='stop'||z.includes('stop-button')||z.includes('stop_response');});"
            + "const statusNow=()=>{let best=null;const all=[...document.querySelectorAll('[role=status],[aria-live],summary,[data-testid]')];for(const e of all){if(!V(e))continue;const txt=N(e.innerText||e.textContent);if(!txt||txt.length>220)continue;const role=(e.getAttribute('role')||'').toLowerCase();const live=(e.getAttribute('aria-live')||'').toLowerCase();const test=(e.getAttribute('data-testid')||'').toLowerCase();const semantic=role==='status'||e.tagName==='SUMMARY'||/reason|think|progress|status|research|browse|search/.test(test)||(live&&txt.length<=160);if(!semantic)continue;let score=0;if(role==='status')score+=5;if(/reason|think|progress|research/.test(test))score+=6;if(e.tagName==='SUMMARY')score+=4;if(live)score+=2;const kind=role==='status'?'role_status':(e.tagName==='SUMMARY'?'summary':(/reason|think/.test(test)?'reasoning_testid':(/progress|status|research|browse|search/.test(test)?'progress_testid':'aria_live')));if(!best||score>best.score)best={text:txt,kind:kind,score:score};}return best;};"
            + "const scan=()=>{try{const a=latestAssistant();let len=0,sig='-';if(a){const t=N(a.innerText||a.textContent);len=t.length;sig=H(t);}if(a&&(len!==lastAssistantLen||sig!==lastAssistantSig)){lastAssistantLen=len;lastAssistantSig=sig;lastChange=Date.now();S('ASSISTANT_CHANGE',{len:len,sig:sig});}const c=statusNow();const st=c?c.text:'';if(st!==lastStatus){lastStatus=st;S('VISIBLE_STATUS',{text:st,len:st.length,kind:c?c.kind:'none',nodes:c?1:0});}const stop=stopNow();const active=stop||(Date.now()-lastChange<2500);const state=(active?'1':'0')+'|'+(stop?'1':'0')+'|'+(c?'1':'0');if(state!==lastState){lastState=state;S('ACTIVITY_STATE',{active:active,stop:stop,statusNodes:c?1:0});}}catch(_){}};"
